 ...
 */
public class VilleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all villes").get("/api/villes").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ville")
                        .post("/api/villes")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"nom\": \"SAMPLE_TEXT\"" + ", \"codePostal\": \"SAMPLE_TEXT\"" + ", \"nbHabitants\": 0" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ville").get("${new_ville_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ville").delete("${new_ville_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ville entity").exec(scn);
